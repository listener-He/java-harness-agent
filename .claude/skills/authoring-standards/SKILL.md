...
| `description:` (frontmatter) | yes | ≤ 300 chars. Form depends on command **type** (Skills route by description pattern-matching; slash commands route by exact name match, so format is driven by *who reads* the description, not how the command is invoked). **Signal-triggered** (fires on hook hint / lifecycle phase / scenario marker / specific input pattern — e.g., `/h-reflect` on `[reflect-hint]`) MUST use trigger-based form: `TRIGGER when X. NOT FOR: Y. Returns Z.` — the TRIGGER clause helps an LLM session decide "is this the right command to suggest?" when it sees the signal. **Intent-triggered** (fires on plain human request — e.g., `/h-status`, `/h-pr`) MUST use concise workflow-style: `Verb-led one-line summary` — picker UI shows this to humans who already typed the command name; brevity > formal structure. See §4.D for examples. |
...
- **`description:` form depends on command type** — slash commands route by exact name match (NOT by description pattern-matching like Skills do). Description is consumed by (a) the slash-command picker UI for humans, and (b) the system reminder skill listing as informational LLM context. Form should match the consumer:
  - **Signal-triggered** (fires on a hook hint, lifecycle phase, scenario marker, or specific input pattern): use trigger-based `TRIGGER when X. NOT FOR: Y. Returns Z.`. The TRIGGER clause helps an LLM session decide "is this the right command to suggest now?" when it sees the signal. Examples: `/h-reflect` (`[reflect-hint]`), `/h-distill` (`[wiki-distill]`), `/h-incident` (Sentry alert), `/h-ci` (CI failure), `/h-archive` (Phase 6 boundary), `/h-fix-bug` (bug report), `/h-design` (HIGH-risk gate), `/h-from-ticket` (ticket URL), `/h-decompose` (PRD/EPIC input), `/h-research` (analyze keyword), `/h-gates` (phase transition), `/h-release` (RELEASE scenario), `/h-distill-from-code` (wiki divergence).
  - **Intent-triggered** (fires on plain human request, no programmatic signal): use concise workflow-style `Verb-led one-line summary`. Picker UI shows this to humans who already typed the command name; brevity > formal structure. Wordy TRIGGER blocks here add noise without payoff. Examples: `/h-status`, `/h-resume`, `/h-brief`, `/h-pr`, `/h-collab`, `/h-collab-update`, `/h-test-handoff`.
  - Both forms MUST be ≤ 300 chars.
...
- **Migration history**: §4.D was introduced 2026-05-30 with an over-broad "trigger-based for ALL commands" rule. On 2026-05-31 (post-F8 review), the rule was refined to the current signal-vs-intent split — 7 of 20 commands (`/h-status`, `/h-resume`, `/h-brief`, `/h-pr`, `/h-collab`, `/h-collab-update`, `/h-test-handoff`) reverted to workflow-style because trigger-based format degraded picker UX for human-typed commands. 13 remain trigger-based. All 20 currently compliant per the differentiated rule.
