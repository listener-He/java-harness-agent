...
A clean Archive is itself a positive sample for `failure_memory` (which until now only collected failures — `record-success` had 0 callers despite a documented interface). Use the deviation summary from Step 2 as the note so future `/h-context-check` and `failure_memory.py summary` queries can reason about what kind of changes archive cleanly.
