...
Close the CI/CD feedback loop. Pulls failed CI run logs (via `gh`), classifies the failure type, routes it into the correct workflow channel (debug task, incident, or warn), and records it in failure_memory so future `/h-context-check` queries surface the pattern in the recurring-failures view. Requires `gh` CLI.
