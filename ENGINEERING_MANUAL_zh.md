...
**微内核 • 双轨制 • 13大角色 • 技能生态 • 6大钩子 • 纯Markdown文件系统**
...
7. [技能生态 (Master Skills)](#7-技能生态-master-skills)
...
graph TD
    ...
    subgraph Scheduler [Agent 微内核调度器]
    ...
    subgraph Mounting [动态挂载区]
        StandardTrack --> Roles[13大角色 Role Matrix]
        StandardTrack --> Skills[技能生态 Skills]
        StandardTrack --> Hooks[6大钩子 Hooks]
    end

    subgraph RAG_FS [虚拟文件系统]
...
graph TD
    ...
    subgraph STANDARD [STANDARD 轨道 - 重型架构]
        ...
        N --> O[Knowledge Extractor 和 Documentation Curator 写回知识图谱]
    end
```
...
```mermaid
graph TD
    Start([开始]) --> Phase1[1_Explorer: 澄清需求与画定边界]

    Phase1 -->|ambiguity_gate.py 通过| Phase2[2_Propose: 设计 API 契约与 DB 结构]

    Phase2 -->|产出 openspec.md| Phase3[3_Review: 寻找确认偏误与未捕获异常]

    Phase3 -->|内部批判结束| Gate4[4_Approval_Gate: WAITING_APPROVAL]
    Phase3 -->|发现致命漏洞 - 回滚| Phase2

    Gate4 -->|人类确认: 继续| Phase5[5_Implement: 复用现有 Utils 后严格编码]

    Phase5 -->|scope_guard.py 防越界通过| Phase6[6_QA_Archive: Linter 然后安全扫描然后提取 WAL]

    Phase6 -->|writeback_gate.py 校验通过| End([任务完成])
    Phase6 -->|shift_left_hook 编译失败 - 最多2次回滚| Phase5
```
...
- **登场英雄**: `@Code Reviewer`, `@Knowledge Extractor`, `@Documentation Curator`, `@Skill Graph Curator`
- **挂载技能 (Skills)**:
  - `code-review-checklist` (检查 N+1 查询等隐患)
  - `wal-documentation-rules` (规范化提取知识碎片)
- **具体操作步骤**:
  1. **代码体检**: 运行 `secrets_linter.py` 扫描是否泄露了阿里云 AK 等密码（通过 `guard_hook` 执行）。
...
> **注 (v2)**: 安全哨兵角色已从 STANDARD 工作流的阶段挂载中移除。`secrets_linter.py` 现在作为 `guard_hook` 固定步骤在每次代码变更时执行。角色定义保留以供 Scenario A (紧急热修复) 显式引用。
...
sequenceDiagram
    ...
    Hook->>Gate: 运行 scope_guard.py 和 secrets_linter.py
...
## 7. 技能生态 (Master Skills)

传统的 Agent Prompt 往往是一锅粥。本项目将工程纪律拆解为多类别技能系统，存放在 `.agents/skills/` 中，以 `trae-skill-index` 为全局路由表。

### 7.1 默认启用技能 (按生命周期阶段自动调用)
...
| **排障与流程** | `systematic-debugging` | 标准化排障流：复现 -> 定位 -> 修复 -> 防范。 | DEBUG 场景 |
...
面向 AI 驱动的工程工作流：`ai-pipeline`, `blueprint`, `architecture-decision-records`, `eval-harness`, `external-research`, `self-improve`, `ai-slop-cleaner`

### 7.3 工作流与协作技能

Git 工作树、并行代理、发布和知识持久化：`dispatching-parallel-agents`, `using-git-worktrees`, `release`, `deepinit`, `remember`, `writing-plans`, `brainstorming`, `verify`, `test-driven-development`, `ultraqa`, `skill-creator`
...
- `secrets_linter.py`: 扫描硬编码的密码/密钥。通过 `guard_hook` 在每次代码变更时触发（特别是紧急修复）。
