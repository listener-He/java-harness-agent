...
Read: [Project-Level Rules](project-rules.md).

**Step 2 (Drill Down from Knowledge Graph, No Blind Search)**\
Read: [Knowledge Graph Root](llm_wiki/sitemap.md), then drill down layer by layer through indices to your target domain (API / Data / Domain / Architecture / Specs / Preferences).

**Step 3 (Run One Minimal Closed-Loop)**\
Follow: [Lifecycle State Machine](harness/lifecycle.md) to complete one task: Explorer → Propose → Review → Approval → Implement → QA → Archive.

---

## 0.1 How to Use (Multi-Scenario Examples)

This section provides typical "follow-along" playbooks. The rules remain consistent: first drill down from the [Knowledge Graph Root](llm_wiki/sitemap.md) to domain indices, then produce contracts and phased deliverables, and finally write back to indices and archive Specs during the Archive phase.
...
- **Optional Tools (report only, no file modification)**:
  - Graph health check: [wiki_linter.py](scripts/wiki/wiki_linter.py) (dead links/orphans/length warnings)
  - Contract health check: [schema_checker.py](scripts/wiki/schema_checker.py) (critical structure missing checks)
  - Preference health check: [pref_tag_checker.py](scripts/wiki/pref_tag_checker.py) (rule tag规范 checks)
...
```text
.
├── intent/                  # Intent gateway & context funnel (entry & navigation rules)
├── harness/                 # Lifecycle state machine & hooks (interception/correction/closed-loop)
├── llm_wiki/                # Knowledge base (sitemap/index/domain knowledge/archive)
├── skills/                  # Specialized skills (SKILL.md as unit)
└── scripts/                 # Deterministic scripts (graph checks, auxiliary tools)
```
...
### 2.1 project-rules.md (Project-Level Rule Entry)
...
- **Key Files**
  - [intent-gateway.md](intent/intent-gateway.md)
    ...
    - **Output**: `intent/catalog/launch_spec_*.md` (intent queue persistence + breakpoint resumption).
...
- **Key Files**
  ...
  - schema/
    - [schema/index.md](llm_wiki/schema/index.md): Specification domain index (router), telling you "which contract to read / which process to jump to".
    - [openspec_schema.md](llm_wiki/schema/openspec_schema.md): OpenSpec contract template (primary backend deliverable; optionally carries frontend/QA handoff fields).
  - wiki/
...
- **Basic Convention**: One directory per skill, entry file is `skills/<skill-name>/SKILL.md`.
...
**intent/**
- Typical path: User requirements → read llm_wiki/sitemap.md → trigger intent mapping → write to intent/catalog/launch_spec_*.md
...
**Specification File**: [Intent Gateway](intent/intent-gateway.md)
...
**Specification File**: [Context Funnel](intent/context-funnel.md)
...
**Specification File**: [Lifecycle](harness/lifecycle.md)
...
**Specification File**: [Hooks](harness/hooks.md)
...
Contract template details: [OpenSpec Schema](llm_wiki/schema/openspec_schema.md).
...
- Graph health check (dead links/orphans/length warnings): [wiki_linter.py](scripts/wiki/wiki_linter.py)
- Contract structure health check (critical structure missing checks): [schema_checker.py](scripts/wiki/schema_checker.py)
- Preference tag health check (rule tag规范 checks): [pref_tag_checker.py](scripts/wiki/pref_tag_checker.py)
- Lifecycle queue auxiliary (optional): [engine.py](scripts/harness/engine.py)
...
|----------------|-------------------|-------------------|
| OpenSpec (Contract-First) | Freeze requirements and design with structured contracts first, then allow implementation and testing | `llm_wiki/schema/openspec_schema.md` + Phase 2 Propose + Phase 3.5 Approval |
| Harness (Lifecycle/State Machine) | Processes are not verbal agreements, but rollbackable, interceptable, closed-loop state machines | `harness/lifecycle.md` |
| Hooks (Correction System) | Use guard/fail/loop to lock "unauthorized access, runaway, bloat" into the process | `harness/hooks.md` |
| LLM Wiki (Evolvable Knowledge Base) | Use sitemap + multi-level indices to let Agents autonomously roam for retrieval; use archive to prevent bloat | `llm_wiki/sitemap.md` + `llm_wiki/wiki/*/index.md` + `llm_wiki/archive/` |
| Knowledge Graph (Connectivity) | All knowledge must be traceable from root; orphans/dead links considered "garbage" | sitemap mounting discipline + scripts/wiki/wiki_linter.py (optional) |
| Skills (Expert Capability Matrix) | Delegate professional problems to specialized skills, invoked on-demand within process phases, ensuring consistent standards | `skills/*/SKILL.md` + Phase mapping table |
| Engine (Optional Auxiliary) | Does not replace Agent, only provides deterministic hosting of "queue/phase/retry count" for complex tasks | `scripts/harness/engine.py` (optional) |
| Script Tools (Deterministic Enhancement) | Deterministic checks and auxiliaries (graph health checks, index splitting suggestions) delegated to scripts | `scripts/wiki/*` |
...
- Rule Entry: [project-rules.md](project-rules.md)
- Knowledge Graph Root Entry: [llm_wiki/sitemap.md](llm_wiki/sitemap.md)
...
- Lifecycle: [harness/lifecycle.md](harness/lifecycle.md)
