...
| 文档类型 | 路径 |
|---------|------|
| 表定义文档 | `.agents/docs/db/{表名}.md` |
| 建表SQL | `src/main/resources/sql/{模块名}_tables.sql` |
| 模块描述 | `.agents/docs/module_description/{模块名}.md` |
...
1. **创建表定义文档** `.agents/docs/db/{表名}.md`
   - 表说明
   - 字段定义（字段名、类型、必填、说明）
   - 索引定义
   - 建表SQL
...
3. **更新模块描述文档** `.agents/docs/module_description/{模块名}.md`
   - 在「数据表清单」章节添加表引用
   - 在「表关系图」章节更新 ER 图
...
1. **更新表定义文档** `.agents/docs/db/{表名}.md`
   - 修改字段定义表格
   - 更新建表SQL
...
1. **删除表定义文档** `.agents/docs/db/{表名}.md`
