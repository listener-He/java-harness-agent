...
description: "Three pure-local tools (BM25 wiki search, Java symbol index, failure memory) that provide zero-cost context without reading files or consuming token budget. Use before any manual navigation or focus-card authoring."
---
...
**What:** BM25 keyword search over all `.agents/llm_wiki/**/*.md` files. Returns ranked
...
```bash
# Basic search
python3 .agents/scripts/local_intel/wiki_search.py --query "API tenant isolation" --top 3

# JSON output for scripting
python3 .agents/scripts/local_intel/wiki_search.py --query "migration gate DDL" --json

# Rebuild index (after adding new wiki docs)
python3 .agents/scripts/local_intel/wiki_search.py --rebuild
```
...
**Index location:** `.agents/local_intel/wiki_bm25.json` (gitignored, rebuilt on demand)
...
python3 .agents/scripts/local_intel/code_index.py --build

# Who calls this method? (for blast radius estimation)
python3 .agents/scripts/local_intel/code_index.py --who-calls createOrder

# Which mapper XMLs touch this table? (for Scenario B: DB migration)
python3 .agents/scripts/local_intel/code_index.py --what-touches-table orders

# Full impact analysis of a file change (importers + callers)
python3 .agents/scripts/local_intel/code_index.py --impact-of src/main/java/com/example/service/OrderService.java

# Find symbol by name fragment
python3 .agents/scripts/local_intel/code_index.py --symbol OrderRepository

# Index status
python3 .agents/scripts/local_intel/code_index.py --status
```
...
**Index location:** `.agents/local_intel/code_index.json` (gitignored, rebuild with `--build`)
...
python3 .agents/scripts/local_intel/failure_memory.py query \
...
python3 .agents/scripts/local_intel/failure_memory.py record-success \
...
python3 .agents/scripts/local_intel/failure_memory.py stats
```
...
**Storage:** `.agents/local_intel/failure_memory.json` (gitignored, max 500 records FIFO)
...
# Verify
python3 .agents/scripts/local_intel/wiki_search.py --status
python3 .agents/scripts/local_intel/code_index.py --status
```

Add to `.agents/scripts/tools/bootstrap.sh` or run manually after large refactors.
Indexes are stored in `.agents/local_intel/` (gitignored).
