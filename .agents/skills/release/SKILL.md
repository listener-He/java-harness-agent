---
name: release
description: Generic release assistant — analyzes repo release rules, stores them under .agents/workflow/runs/, then guides the release
---
...
A thin, repo-aware release assistant. On first run it inspects the project and CI to derive release rules, stores them under `.agents/workflow/runs/` as a runtime artifact, then walks you through a release using those rules.
...
Check whether a cached release rule artifact exists under `.agents/workflow/runs/` (e.g., `<YYYY-MM-DD>_<slug>_release_rule.md`).
...
Inspect the repo and answer the following. Write answers into the release rule artifact under `.agents/workflow/runs/`.
...
### Step 2 — Write Release Rule Artifact

Create or overwrite the release rule artifact with this structure:
...
- The release rule artifact under `.agents/workflow/runs/` is runtime-only. If you want to persist it, archive it to `.agents/llm_wiki/archive/` as part of the Archive phase.
