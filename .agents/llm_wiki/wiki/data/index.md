...
- During `Archive` (STANDARD only), the Agent MUST extract table changes from `<YYYY-MM-DD>_<slug>_task_brief.md` and append them to the table below.
...
Append a new row during `Archive` (STANDARD only) using the template below.
