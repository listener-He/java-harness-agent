...
## 契约模板（Schema）
- **[OpenSpec 契约模板 (OpenSpec Schema)](openspec_schema.md)**：后端提案统一格式。*如需并行协作*，其中的 API Contract 与 Acceptance Criteria 可作为前端/QA 的可选交接物（包含 JSON Example 与 Given/When/Then）。
...
## 链接规范（统一标准）
- 本知识库内链接优先使用相对路径，避免把目录名（如 `.agents/`）写进相对链接导致断头链接。
