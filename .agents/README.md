...
**第一步（必读规则入口）**\
阅读：[AGENTS.md（统一入口）](../AGENTS.md)。

**第二步（从知识图谱下钻，不盲搜）**\
阅读：[知识图谱根节点](llm_wiki/KNOWLEDGE_GRAPH.md)，然后按索引逐层下钻到你要的域（API / Data / Domain / Architecture / Specs / Preferences）。

**第三步（跑一次最小闭环）**\
按：[生命周期状态机](workflow/LIFECYCLE.md) 完成一次任务：Explorer → Propose → Review → Approval → Implement → QA → Archive。

## 0.1 如何使用（多场景示例）

本节给出“照着跑”的典型剧本。规则保持一致：先从 [知识图谱根节点](llm_wiki/KNOWLEDGE_GRAPH.md) 下钻到域索引，再产出契约与阶段性工件，最后在 Archive 阶段反向写回索引并归档 Spec。
...
```text
.
├── router/                  # 意图网关与上下文漏斗（入口与导航规则）
├── workflow/                 # 生命周期状态机与钩子（拦截/纠偏/闭环）
...
- 典型路径：用户需求 → 读取 llm\_wiki/KNOWLEDGE_GRAPH.md → 触发意图映射 → 写入 router/runs/launch\_spec\_\*.md
...
**规范文件**：[知识漏斗](router/CONTEXT_FUNNEL.md)
...
**规范文件**：[生命周期](workflow/LIFECYCLE.md)
...
- 生命周期队列辅助（可选）：[engine.py](scripts/workflow/engine.py)
...
| Harness（生命周期/状态机） | 流程不是口头约定，而是可回退、可拦截、可闭环的状态机                        | `workflow/LIFECYCLE.md`                                                      |
| Hooks（纠偏系统）       | 用 guard/fail/loop 把“越权、暴走、膨胀”锁死在流程里               | `workflow/HOOKS.md`                                                          |
| LLM Wiki（可演进知识库）  | 用 sitemap + 多级 index 让 Agent 自主游走检索；用 archive 防膨胀 | `llm_wiki/KNOWLEDGE_GRAPH.md` + `llm_wiki/wiki/*/index.md` + `llm_wiki/archive/`    |
...
| Engine（可选辅助）      | 不替代 Agent，只在复杂任务时提供“队列/阶段/重试计数”的确定性托管             | `scripts/workflow/engine.py`（可选）                                             |
...
- 知识图谱根入口：[llm\_wiki/KNOWLEDGE_GRAPH.md](llm_wiki/KNOWLEDGE_GRAPH.md)
- 契约模板：[llm\_wiki/schema/openspec\_schema.md](llm_wiki/schema/openspec_schema.md)
- 意图网关：[router/ROUTER.md](router/ROUTER.md)
- 知识漏斗：[router/CONTEXT_FUNNEL.md](router/CONTEXT_FUNNEL.md)
- 生命周期：[workflow/LIFECYCLE.md](workflow/LIFECYCLE.md)
- Hooks：[workflow/HOOKS.md](workflow/HOOKS.md)
